...
namespace RoMan
{
	...
	std::pair<Ref<TextureCube>, Ref<TextureCube>> SceneRenderer::CreateEnvironmentMap(const std::string& filepath)
	{
		...
		envEquirect->Bind();
		RM_RENDER_3(envUnfiltered, cubemapSize, envEquirect, {
		...
		RM_RENDER_2(envUnfiltered, envFiltered, {
		...
		RM_RENDER_3(cubemapSize, envFiltered, envFilteringShader,
			{
	...
	{
		RM_CORE_ASSERT(false, "Not implemented");
		return nullptr;
	}
