...
namespace RoMan
{
	...
	{
		if (Input::IsKeyPressed(RM_KEY_LEFT_ALT))
		{
			const glm::vec2& mousePos{ Input::GetMouseX(), Input::GetMouseY() };
			glm::vec2 delta = mousePos - m_InitialMousePosition;
			m_InitialMousePosition = mousePos;

			delta *= ts.GetSeconds();

			if (Input::IsMouseButtonPressed(RM_MOUSE_BUTTON_MIDDLE))
			{
				MousePan(delta);
			}
			else if (Input::IsMouseButtonPressed(RM_MOUSE_BUTTON_LEFT))
			{
				MouseRotate(delta);
			}
			else if (Input::IsMouseButtonPressed(RM_MOUSE_BUTTON_RIGHT))
			{
				MouseZoom(delta.y);
			}
		}
